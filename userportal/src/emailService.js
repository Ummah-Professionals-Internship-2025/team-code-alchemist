...
export const sendMeetingEmails = async (meetingData, selectedDate) => {
  ...
  if (!EMAILJS_CONFIG.publicKey || (!EMAILJS_CONFIG.gmailServiceId && !EMAILJS_CONFIG.outlookServiceId) || !EMAILJS_CONFIG.menteeTemplateId || !EMAILJS_CONFIG.mentorTemplateId) {
    return { success: false, error: 'Missing EmailJS configuration. Ensure REACT_APP_EMAILJS_* env vars are set.' };
  }
  ...
  try {
    mentee = await emailjs.send(
      ...
      { mentee_name: meetingData.menteeName, mentor_name: meetingData.mentorName, meeting_date: formattedDate, meeting_time: meetingData.meetingTime, mentee_email: meetingData.menteeEmail }
    );
  } catch (e) {
  ...
  try {
    mentor = await emailjs.send(
      ...
      { mentor_name: meetingData.mentorName, mentee_name: meetingData.menteeName, meeting_date: formattedDate, meeting_time: meetingData.meetingTime, mentor_email: meetingData.mentorEmail }
    );
  } catch (e) {
