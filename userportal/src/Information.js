...
const Information = () => {
  ...
  const fetchMeetings = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:3003/api/meetings/mentee/${user.uid}/past`);
    ...
    <div style={{ padding: '24px', color: '#00212C', background: '#ffffff', minHeight: '100vh', width: '100%', boxSizing: 'border-box' }}>
      ...
      {pastMeetings.length > 0 ? (
        <div style={{ 
          ...
          <h3 style={{ color: '#007CA6', fontWeight: 700, fontSize: 20, marginBottom: 16 }}>Past Meetings</h3>
