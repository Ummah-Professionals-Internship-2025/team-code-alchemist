  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (emailResult?.success) {
        ...
        window.alert(`Email sending failed: ${emailResult?.error || 'Unknown error'}`);
      }
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          <p>Please wait while we analyze your profile and find the perfect mentors for you.</p>
        </div>
      </div>
    );
  }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <h2 style={{ fontSize: 28, fontWeight: 700, color: '#007CA6', margin: 0 }}>
          ...
          <div style={{ display: 'grid', gap: 20 }}>
            {topMentors.map((match, index) => (
              <div key={match.mentor.id} style={{
                border: selectedMentor?.mentor.id === match.mentor.id ? '2px solid #007CA6' : '1px solid #ddd',
                borderRadius: 8,
                padding: 20,
                background: selectedMentor?.mentor.id === match.mentor.id ? '#f0f8ff' : '#fff',
                cursor: 'pointer',
                transition: 'all 0.2s'
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
                  <div>
                    <h3 style={{ fontSize: 20, fontWeight: 600, marginBottom: 8, color: '#007CA6' }}>
                      {match.mentor.name} #{index + 1}
                    </h3>
                    <div style={{
                      background: getScoreColor(calculatePercentage(match.totalScore, TOTAL_POSSIBLE_SCORE)),
                      color: '#fff',
                      padding: '4px 12px',
                      borderRadius: 16,
                      fontSize: 14,
                      ...
                      Match Score: {calculatePercentage(match.totalScore, TOTAL_POSSIBLE_SCORE)}%
                    </div>
                  </div>
                  <button style={{
                    background: selectedMentor?.mentor.id === match.mentor.id ? '#007CA6' : '#e0e0e0',
                    color: selectedMentor?.mentor.id === match.mentor.id ? '#fff' : '#666',
                    border: 'none',
                    padding: '8px 16px',
                    borderRadius: 6,
                    cursor: 'pointer',
                    ...
                    {selectedMentor?.mentor.id === match.mentor.id ? 'Selected' : 'Select'}
                  </button>
                </div>

                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12, marginBottom: 16 }}>
                  <div><strong>Experience:</strong> {formatExperience(match.mentor.yearsOfExperience)}</div>
                  <div><strong>Companies:</strong> {match.mentor.companies}</div>
                  <div><strong>Major:</strong> {match.mentor.major || 'Not specified'}</div>
                  <div><strong>Industry:</strong> {Array.isArray(match.mentor.industry) ? match.mentor.industry.join(', ') : match.mentor.industry || 'Not specified'}</div>
                  <div><strong>Skills:</strong> {match.mentor.skills}</div>
                  <div><strong>Can Help With:</strong> {match.mentor.helpIn}</div>
                </div>

                {/* Score Breakdown with Percentages */}
                <div style={{ background: '#f5f5f5', padding: 12, borderRadius: 6, marginBottom: 16 }}>
                  <h4 style={{ fontSize: 14, fontWeight: 600, marginBottom: 8, color: '#333' }}>Why This Match?</h4>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: 8, fontSize: 12 }}>
                    <div>Major: {calculatePercentage(match.breakdown.major, 40)}%</div>
                    <div>Industry: {calculatePercentage(match.breakdown.industry, 35)}%</div>
                    <div>Skills: {calculatePercentage(match.breakdown.skills, 20)}%</div>
                    <div>Services: {calculatePercentage(match.breakdown.helpIn, 5)}%</div>
                    <div>Company Size: {calculatePercentage(match.breakdown.companySize, 5)}%</div>
                  </div>
                </div>

                {/* Calendar View for Availability */}
                {selectedMentor?.mentor.id === match.mentor.id && match.mentor.availability && (
                  <div style={{ marginTop: 16 }}>
                    <h4 style={{ fontSize: 16, fontWeight: 600, marginBottom: 12, color: '#333' }}>Available Times:</h4>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                      <button 
                        ...
                        disabled={(() => {
                          ...
                          const currentMonthNum = currentDate.getMonth();
                          const currentYear = currentDate.getFullYear();
                          return currentMonth.getMonth() <= currentMonthNum && currentMonth.getFullYear() <= currentYear;
                        })()}
                        style={{
                          background: (() => {
                            const currentDate = new Date();
                            const currentMonthNum = currentDate.getMonth();
                            const currentYear = currentDate.getFullYear();
                            return currentMonth.getMonth() <= currentMonthNum && currentMonth.getFullYear() <= currentYear ? '#ccc' : '#f0f0f0';
                          })(),
                          border: 'none',
                          padding: '8px 12px',
                          borderRadius: 4,
                          cursor: (() => {
                            ...
                            const currentMonthNum = currentDate.getMonth();
                            const currentYear = currentDate.getFullYear();
                            return currentMonth.getMonth() <= currentMonthNum && currentMonth.getFullYear() <= currentYear ? 'not-allowed' : 'pointer';
                          })()
                        }}
                      >
                        ←
                      ...
                      <button 
                        ...
                        disabled={(() => {
                          ...
                          const currentMonthNum = currentDate.getMonth();
                          const currentYear = currentDate.getFullYear();
                          return currentMonth.getMonth() >= currentMonthNum + 1 && currentMonth.getFullYear() >= currentYear;
                        })()}
                        style={{
                          background: (() => {
                            const currentDate = new Date();
                            const currentMonthNum = currentDate.getMonth();
                            const currentYear = currentDate.getFullYear();
                            return currentMonth.getMonth() >= currentMonthNum + 1 && currentMonth.getFullYear() >= currentYear ? '#ccc' : '#f0f0f0';
                          })(),
                          border: 'none',
                          padding: '8px 12px',
                          borderRadius: 4,
                          cursor: (() => {
                            ...
                            const currentMonthNum = currentDate.getMonth();
                            const currentYear = currentDate.getFullYear();
                            return currentMonth.getMonth() >= currentMonthNum + 1 && currentMonth.getFullYear() >= currentYear ? 'not-allowed' : 'pointer';
                          })()
                        }}
                      >
                        →
                    ...
                    <div style={{ 
                      ...
                      {(() => {
                        ...
                        for (let i = 0; i < startingDay; i++) {
                          days.push(<div key={`empty-${i}`} style={{ padding: '8px 4px' }} />);
                        }
                    ...
                    {selectedDate && selectedTime && (
                      <div style={{ 
                        ...
                        <strong>Selected Meeting:</strong> {selectedDate.toLocaleDateString('en-US', { 
                          weekday: 'long', 
                          year: 'numeric', 
                          month: 'long', 
                          day: 'numeric' 
        ...
        {selectedMentor && selectedDate && selectedTime && (
          <div style={{ marginTop: 24, textAlign: 'center' }}>
            ...
            >
              {loading ? 'Scheduling...' : `Schedule Meeting with ${selectedMentor.mentor.name}`}
            </button>
          </div>
        )}
      </div>
