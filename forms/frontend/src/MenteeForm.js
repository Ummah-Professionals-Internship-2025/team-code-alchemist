    ...
    return (
      <div className="container">
        <div className="form-card">
          ...
          <p>Redirect in 10 secs, login with the email and password you inputted in the form.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="form-card scrollable-form">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Select
            ...
            value={industryOptions.filter(opt => form.industry.includes(opt.value))}
            onChange={handleIndustryChange}
            classNamePrefix="react-select"
            placeholder="Select industry..."
            required
          />
          <div style={{fontSize: '0.95rem', color: '#888', marginBottom: 12}}>You can select multiple industries.</div>
          ...
          <Select
            ...
            onChange={selected => setForm({ ...form, major: selected ? selected.value : '' })}
            classNamePrefix="react-select"
          ...
          <label>Current grade <span style={{color: 'red'}}>*</span></label>
          <select name="currentGrade" value={form.currentGrade} onChange={handleChange} required>
          ...
          <label>What service are you looking for? <span style={{color: 'red'}}>*</span></label>
          <select name="serviceLookingFor" value={form.serviceLookingFor} onChange={handleChange} required>
          ...
          <input name="github" value={form.github || ''} onChange={handleChange} placeholder="GitHub profile URL" />

          <label>LinkedIn (optional)</label>
          <input name="linkedin" value={form.linkedin || ''} onChange={handleChange} placeholder="LinkedIn profile URL" />

          <label>Time zone (optional)</label>
          <input name="timeZone" value={form.timeZone || ''} onChange={handleChange} placeholder="e.g. EST, PST, GMT+3" />

          <label>Country (optional)</label>
          <input name="country" value={form.country || ''} onChange={handleChange} placeholder="Country" />

          <label>University (optional)</label>
          <input name="university" value={form.university || ''} onChange={handleChange} placeholder="University name" />
          ...
          {form.resume && <div style={{marginBottom: 16, fontSize: '1.1rem'}}>Selected file: {form.resume.name}</div>}
          ...
          <div style={{display: 'flex', alignItems: 'center', marginBottom: 8}}>
            ...
            />
            <button type="button" style={{marginLeft: 8, border: 'none', background: 'none', cursor: 'pointer'}} onClick={() => setShowPassword(v => !v)}>
          ...
          {showPasswordFeedback && (
            <div style={{marginBottom: 12}}>
              <div style={{height: 8, background: '#eee', borderRadius: 4, marginBottom: 4}}>
                <div style={{width: `${(passwordStrength.score/5)*100}%`, height: '100%', borderRadius: 4, background: passwordStrength.label === 'Strong' ? '#4caf50' : passwordStrength.label === 'Medium' ? '#ff9800' : '#f44336', transition: 'width 0.3s'}}></div>
              </div>
              <div style={{fontWeight: 'bold', color: passwordStrength.label === 'Strong' ? '#4caf50' : passwordStrength.label === 'Medium' ? '#ff9800' : '#f44336'}}>
                {passwordStrength.label} password
              </div>
              <ul style={{fontSize: '0.95rem', margin: '8px 0 0 0', paddingLeft: 18}}>
                <li style={{color: passwordStrength.requirements.length ? '#4caf50' : '#f44336'}}>At least 8 characters</li>
                <li style={{color: passwordStrength.requirements.upper ? '#4caf50' : '#f44336'}}>At least one uppercase letter</li>
                <li style={{color: passwordStrength.requirements.lower ? '#4caf50' : '#f44336'}}>At least one lowercase letter</li>
                <li style={{color: passwordStrength.requirements.special ? '#4caf50' : '#f44336'}}>At least one special character</li>
              </ul>
            </div>
          )}
          ...
          <div style={{display: 'flex', alignItems: 'center', marginBottom: 16}}>
            <input
              ...
              required
              style={{flex: 1}}
              onFocus={() => { setConfirmPasswordTouched(true); setConfirmPasswordFocused(true); }}
              onBlur={() => setConfirmPasswordFocused(false)}
            />
            <button type="button" style={{marginLeft: 8, border: 'none', background: 'none', cursor: 'pointer'}} onClick={() => setShowConfirmPassword(v => !v)}>
              <span role="img" aria-label="Show password">{showConfirmPassword ? '🙈' : '👁️'}</span>
            </button>
          </div>
          {confirmPasswordFocused && form.confirmPassword && (
            <div style={{color: form.password === form.confirmPassword ? '#4caf50' : '#f44336', marginBottom: 12, fontWeight: 'bold'}}>
