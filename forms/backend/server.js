...
app.post('/api/mentee', upload.single('resume'), async (req, res) => {
  try {
    ...
    try {
      const existingOAuthDoc = await db.collection('mentees').doc(form.email).get();
      ...
      console.log('No existing OAuth tokens found or error checking:', error.message);
    }
...
app.post('/api/meetings', async (req, res) => {
  try {
    ...
    if (!meetingData.mentorId) {
      return res.status(400).json({ error: 'mentorId is required for meeting creation' });
    }
  ...
  try {
    ...
    try {
      ...
      if (mentorToken.expiry_date && now > mentorToken.expiry_date) {
        await refreshOAuthToken(meetingData.mentorId, mentorToken.refresh_token);
      }
