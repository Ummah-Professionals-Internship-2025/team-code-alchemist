        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
          <thead>
            <tr style={{ backgroundColor: '#f5f5f5' }}>
              <th style={{ padding: '8px', border: '1px solid #ddd', textAlign: 'left', fontWeight: 'bold' }}>Time</th>
              {days.map(day => (
                <th key={day} style={{ padding: '8px', border: '1px solid #ddd', textAlign: 'center', fontWeight: 'bold', fontSize: '0.8rem' }}>
                  {day}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {timeSlots.map(time => (
              <tr key={time}>
                <td style={{ padding: '6px 8px', border: '1px solid #ddd', fontWeight: 'bold', fontSize: '0.8rem' }}>
                  {time}
                </td>
                {days.map(day => (
                  <td key={`${day}-${time}`} style={{ padding: '4px', border: '1px solid #ddd', textAlign: 'center' }}>
                    <button
                      type="button"
                      style={{
                        padding: '4px 8px',
